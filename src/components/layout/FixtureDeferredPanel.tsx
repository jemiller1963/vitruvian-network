...
export function FixtureDeferredPanel({ section }: FixtureDeferredPanelProps) {
  ...
  return (
    <section className="glass-card p-6 sm:p-8" aria-labelledby="deferred-screen-title">
      <div className="flex max-w-2xl items-start gap-4">
        <div className="rounded-lg bg-amber-500/10 p-3 text-amber-400" aria-hidden="true">
          <DatabaseZap className="h-5 w-5" />
        </div>
        <div className="space-y-2">
          <p className="text-xs font-semibold uppercase tracking-wider text-amber-400">Deferred screen</p>
          <h1 id="deferred-screen-title" className="font-heading text-xl font-semibold text-dvn-text-primary">
            {content.title} is intentionally disconnected
          </h1>
          ...
          <p className="text-sm leading-6 text-dvn-text-muted">
            No legacy Supabase request has been made. Navigation remains available while this screen awaits migration.
          </p>
        </div>
      </div>
    </section>
